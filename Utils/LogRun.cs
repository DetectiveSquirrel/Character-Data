...
public class LogRun
{
    ...
    {
        CharacterData.MainPlugin.LogMessage(DataToString(), timer);
    }
    ...
    public string DataToString(bool full = false)
    {
        try
        {
            ...
            var str2 = str1 + string.Format("[Level: {0} ({1:N2}%)] [Gained XP: {2:#,##0} ({3}%)] [Area Dif: {5}] [Areas ETA: {4}/{6}] [Kills: {7:#,##0} | Total Kills: {8:#,##0}]",
                CharacterData.LocalPlayer.Level,
                InstanceSnapshot.Progress(),
                CharacterData.Instance.ExperienceGained(),
                CharacterData.Instance.LevelPercentGained(),
                ...
                CharacterData.LocalPlayer.Kills);
            ...
            if (full)
            {
                str2 += string.Format(" [Area: {0}] [Area Time: {1}]", CharacterData.Instance.JoinArea.DisplayName, CharacterData.Instance.RunTime());
            }
    ...
    public bool NewEntry()
    {
        try
        {
            ...
            {
                while (string.IsNullOrEmpty(CharacterData.MainPlugin.GameController.Game.IngameState.Data.LocalPlayer.GetComponent<Player>().PlayerName))
                {
                    Thread.Sleep(5);
                    CharacterData.MainPlugin.LogError("PlayerData: Null or Empty Name", 10);
                }

                playerName = CharacterData.MainPlugin.GameController.Game.IngameState.Data.LocalPlayer.GetComponent<Player>().PlayerName;
            }
    ...
    public string NeatString(bool full = false)
    {
        try
        {
            ...
            return str + string.Format("Level: {0} ({1:N2}%){7}Area Difference: {5}{7}XP gained: {2:#,##0} ({3}%){7}Area Until Level: {4}/{6}{7}Level Ups: {8}{7}Area Kills: {9:#,##0}",
                CharacterData.LocalPlayer.Level,
                InstanceSnapshot.Progress(),
                CharacterData.Instance.ExperienceGained(),
                CharacterData.Instance.LevelPercentGained(),
                ...
                CharacterData.Instance.TotalRunsToNextLevel(),
                Environment.NewLine,
                CharacterData.LocalPlayer.Level - CharacterData.Instance.JoinLevel,
                CharacterData.LocalPlayer.Kills - CharacterData.Instance.JoinKills);
        }
        catch
        {
