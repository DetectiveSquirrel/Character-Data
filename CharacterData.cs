...
public class CharacterData : BaseSettingsPlugin<CharacterDataSettings>
{
    ...
    public static List<PartyElementWindow> PlayerInPartyDraw { get; set; } = new();
    ...
    public static int TryGetStat(GameStat stat, Entity entity) => entity.Stats.GetValueOrDefault(stat, 0);

    public int TryGetStat(GameStat stat) => GameController.Player.Stats.GetValueOrDefault(stat, 0);
    ...
    public override void DrawSettings()
    {
        if (ImGui.TreeNode("Player Kill Leaderboard"))
        {
            ...
            {
                ImGui.BulletText($"{item.Key}: {item.Value:#,##0}");
            }
    ...
    public override void Render()
    {
        ...
        if (Settings.SoulgainPrev)
        {
            ...
            {
                Graphics.DrawText($"Soul Gain Prevention\n{buffOut.Timer:0.##}", new Vector2(Settings.SoulGainPrevX, Settings.SoulGainPrevY), Settings.SoulGainPrevColor);
            }
        }
        ...
        if (Settings.Wardloop && !GameController.Area.CurrentArea.IsTown)
        {
            ...
            {
                Graphics.DrawText("WARDLOOP BAD", new Vector2(Settings.WardLoopX, Settings.WardLoopY), Settings.WardloopColor, "ProggyClean:46", FontAlign.Center);
            }
        }
    ...
    private void DrawPartyElementStats(PartyElementWindow partyElementWindow)
    {
        ...
        DrawStatText($"Kills: {TryGetStat(GameStat.CharacterKillCount, partyElementWindow.Player):#,##0}", Color.White);
    ...
    private bool ShouldDrawPartyElement(PartyElementWindow partyElementWindow) => partyElementWindow.Player?.IsValid != null &&
    ...
    public static double Progress(Entity entity) => entity.GetComponent<Player>().Level != 100
        ? (entity.GetComponent<Player>().XP - (double)PlayerExperience.TotalExperience[entity.GetComponent<Player>().Level]) / PlayerExperience.NextExperience[entity.GetComponent<Player>().Level] *
          100.0
        : 0.0;
    ...
    private void DrawResistances()
    {
        ...
        var MaxLightRes = TryGetStat(GameStat.MaximumLightningDamageResistancePct) != 0
        ...
        Graphics.DrawText($"{"Fire:",-8}{FireRes,-5}({(num1 > 0 ? "+" + num1 : num1.ToString())})",
            new Vector2(Settings.ResistanceX, Settings.ResistanceY),
            Settings.FireResistanceColor,
            FontAlign.Left);

        Graphics.DrawText($"{"Cold:",-8}{ColdRes,-5}({(num2 > 0 ? "+" + num2 : num2.ToString())})",
            new Vector2(Settings.ResistanceX, Settings.ResistanceY + Settings.ResistanceTextSize),
            Settings.ColdResistanceColor,
            FontAlign.Left);

        Graphics.DrawText($"{"Light:",-8}{LightRes,-5}({(num3 > 0 ? "+" + num3 : num3.ToString())})",
            new Vector2(Settings.ResistanceX, Settings.ResistanceY + Settings.ResistanceTextSize * 2),
            Settings.LightningResistanceColor,
            FontAlign.Left);

        Graphics.DrawText($"{"Chaos:",-8}{ChaosRes,-5}({(num4 > 0 ? "+" + num4 : num4.ToString())})",
            new Vector2(Settings.ResistanceX, Settings.ResistanceY + Settings.ResistanceTextSize * 3),
            Settings.ChaosResistanceColor,
            FontAlign.Left);

        Graphics.DrawText($"{"Kills:",-8}{LocalPlayer.Kills:#,##0}", new Vector2(Settings.ResistanceX, Settings.ResistanceY + Settings.ResistanceTextSize * 4), Settings.KillsColor, FontAlign.Left);
    }
    ...
    private static SortedDictionary<string, int> GetFilteredDeployedObjects(List<DeployedObject> objectList)
    {
        ...
        {
            if (actorDeployedObjects.TryGetValue(deployedObject.Entity.RenderName, out var value))
            {
                actorDeployedObjects[deployedObject.Entity.RenderName] = ++value;
            }
            else
            {
    ...
    private static int ResistanceDifference(object capRes, object uncappedRes, object maxCapRes) => (int)uncappedRes <= (int)capRes
        ? (int)capRes - (int)maxCapRes
        : (int)uncappedRes - (int)capRes;
    ...
    private void DrawGlobePercent(bool toggle, double percent, Vector2 pos, Color backColor, Color textColor)
    {
        ...
        {
            var drawText = !string.IsNullOrEmpty(Settings.GlobeFont)
    ...
    public class LocalPlayer
    {
        ...
        private static Player GetPlayerComponent() =>
            Entity != null && Entity.TryGetComponent(out Player playerComp)
                ? playerComp
                : null;
    }
}